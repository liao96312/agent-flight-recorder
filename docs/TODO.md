...
| 7 | `D0-01` → `D0-06` | CLI 契约与持久信任通过；等待 Desktop 完整重启后的新任务；`D0-02..D0-06` 暂停 | 重启后新桌面任务实际触发五类 AFR 探针事件 |
...
- [ ] **D0-01 [P0] Codex Desktop 实机 hook 契约探针（停止门）**
  ...
  - 2026-07-22 状态：npm Codex 0.145.0 的五事件契约与持久信任已通过；PATH 中 OpenClaw Codex 0.138.0 是此前未发现默认 plugin hooks 的根因。08:59 启动的 Desktop 进程在 Hook 安装后新建任务仍不加载，等待完整重启后完成最终探针。证据见 `D0-01-HOOK-PROBE.md`。
  - 恢复条件：完全重启 Codex Desktop 后新建任务，只重跑一次五事件探针。未满足前不实施 D0-02..D0-06。
...
F0 契约
  ...
  -> REL-04 最终候选一致性检查（已完成）
      ...
      -> [停止门] D0-01 Desktop hook 探针（CLI 已通过；等待 Desktop 重启后新任务）
          -> R0-09 二十次真实会话（逐条记录 capture mode/version/commit）
          -> DEC-01 单方向数据门
              -> [仅证据触发] M4 hooks / 其他一个扩展方向
```

M1 与正式发布门槛已经通过。`D0-01` 的 CLI 契约与持久信任已通过，但未重启的 Desktop 进程在新任务中也不加载新 Hook；`D0-02..D0-06` 暂停，待完整重启后的新任务重跑五事件探针。D0-06 完成后再执行 `R0-09 / DEC-01`；20 次会话没有重复证据前，不扩展 Claude hooks、Hosted tools 适配、历史导入、Dify、索引、签名或团队后台。
