...
| Codex Desktop hooks | 不进入 v0.1；产品负责人于 2026-07-22 明确要求适配桌面端当前任务，先执行有停止门的最小 D0 切片 | `D0-01` 于 2026-07-22 未通过停止门，D0-02..D0-06 暂停；更广 hooks 仍由 `DEC-01` 控制 |
...
| M4a Codex Desktop hooks | D0-01 停止门未通过 | 等待桌面端 `/hooks` 信任与新任务重测 | `afr hook`、hook-only session、并发锁、Desktop E2E | D0-01 实机触发正式事件；D0-06 当前任务无需嵌套 CLI 且 verify 通过 |
...
| 7 | `D0-01` → `D0-06` Codex Desktop 当前任务接入 | D0-01 未通过；后续暂停 | 桌面 `/hooks` 信任后的新任务先通过五事件探针；再恢复严格串行 |
