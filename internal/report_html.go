...
<section><h2>Event timeline</h2><table><thead><tr><th>Seq</th><th>Time</th><th>Type</th><th>Source</th><th>Summary</th></tr></thead><tbody>{{range .View.Timeline.Items}}{{if .OmittedCount}}<tr data-filter-item><td colspan="5" class="muted">{{.OmittedCount}} events omitted (seq {{.OmittedFrom}}–{{.OmittedTo}}); full redacted evidence remains in events.jsonl.</td></tr>{{else}}<tr data-filter-item data-event-row><td>{{.Seq}}</td><td>{{.Timestamp}}</td><td>{{.Type}}</td><td>{{.Source}}</td><td><code>{{.Summary}}</code>{{if .SummaryTruncated}} <span class="muted">[summary truncated]</span>{{end}}</td></tr>{{end}}{{end}}</tbody></table></section>
